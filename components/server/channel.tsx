...
const iconMap = {
  [ChannelType.TEXT]: (
    <Hash className="flex-shrink-0 text-zinc-500 dark:text-zinc-400 w-5 h-5" />
  ),
...
const ServerChannel: React.FC<ServerChannelProps> = ({
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "group px-2 py-2 rounded-md flex items-center gap-x-2 w-full text-zinc-500 dark:text-zinc-400 hover:bg-zinc-700/10 dark:hover:bg-zinc-700/50 transition mb-1",
